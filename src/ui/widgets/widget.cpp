...
std::optional<gui_id> Widget::find_widget(Vec2i16 pos) {
    ...
    util::println("Searching in relative position @ ({}, {}). Widget absolute anchor position ({}, {}) - i.e we are "
                  "looking at absolute pixel position ({}, {})",
                  pos.x, pos.y, abs.x, abs.y, (pos + abs).x, (pos + abs).y);
    ...
    if (is_within_bounds(bounding_rect(), pos + m_pos)) {
        ...
        if (it != std::end(m_children)) {
            ...
            util::println("Pos ({}, {}) in widget: {}, also is in widget {}, at it's relative position ({}, {})", pos.x,
                          pos.y, m_id.id, (*it)->m_id.id, rel_pos.x, rel_pos.y);

            auto t = abs + pos;
            util::println("Searching for children at relative position ({}, {}) in widget {} - Abs position: {}, {}",
                          rel_pos.x, rel_pos.y, (*it)->m_id.id, t.x, t.y);
...
Vec2i16 Widget::calculate_absolute_position() {
    if (p_parent != nullptr) {
        m_screen_pos = calculate_absolute_position() + m_pos;
        return m_screen_pos;
    } else
        return m_pos;
}
...
void Widget::set_fixed_size(std::optional<i16> maybe_width, std::optional<i16> maybe_height) {
    m_fixed_width = maybe_width;
    m_fixed_height = maybe_height;
}

void Widget::handle_mouse_click(Vec2i16 mousePos) {
    util::println("Handle mouse click not implemented on Widget base class. GUIID: {}", this->get_guid().id);
    std::abort();
}
